...
template <typename Field, typename T = typename Field::value_type>
constexpr field_error_handler<Field> ignore_handler =
    [](T v) -> T { return T{0}; };
...
constexpr auto truncate_handler(T v) -> T {
    ...
    return static_cast<T>(detail::to_underlying(v) &
                          (Field::mask >> Field::lsb()));
}

template <typename Field>
constexpr field_error_handler<Field> handle_field_error =
    truncate_handler<Field>;

template <typename Register, typename T = typename Register::value_type>
using register_error_handler = T (*)(T);

template <typename Register, typename T = typename Register::value_type>
constexpr register_error_handler<Register> mask_handler =
    [](T v) -> T { return static_cast<T>(v & Register::writable_mask); };
